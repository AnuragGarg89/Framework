...
public class GLP_311006_VerifyNextContentAfterForwardArrowClickOnTOC extends
		BaseClass {
	public GLP_311006_VerifyNextContentAfterForwardArrowClickOnTOC() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that Next content as per TOC should be displayed when learner taps on forward arrow")
	public void verifyNextContentAfterForwardArrowClickOnTOC() {
		startReport(
				getTestCaseId(),
				"To verify that Next content as per TOC should be displayed when learner taps on forward arrow");
