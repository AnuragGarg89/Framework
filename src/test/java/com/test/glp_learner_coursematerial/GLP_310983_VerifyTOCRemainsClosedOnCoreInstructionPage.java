...
public class GLP_310983_VerifyTOCRemainsClosedOnCoreInstructionPage extends
		BaseClass {
	public GLP_310983_VerifyTOCRemainsClosedOnCoreInstructionPage() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that TOC should remain closed if Student navigates to next page using arrow button if the TOC is already closed")
	public void verifyTOCRemainsClosedOnCoreInstructionPage() {
		startReport(
				getTestCaseId(),
				"To verify that TOC should remain closed if Student navigates to next page using arrow button if the TOC is already closed.");
		...
		try {
			...
			// Click on Drawer ICON to Close TOC
