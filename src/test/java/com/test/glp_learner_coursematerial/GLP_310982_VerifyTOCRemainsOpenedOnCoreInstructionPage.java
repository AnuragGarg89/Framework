...
public class GLP_310982_VerifyTOCRemainsOpenedOnCoreInstructionPage extends
		BaseClass {
	public GLP_310982_VerifyTOCRemainsOpenedOnCoreInstructionPage() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that TOC should remain opened if Student navigates to next page either from TOC or using arrow button if the TOC is already opened.")
	public void verifyTOCRemainsOpenedOnCoreInstructionPage() {
		startReport(
				getTestCaseId(),
				"To verify that TOC should remain opened if Student navigates to next page either from TOC or using arrow button if the TOC is already opened.");
		...
		try {
			...
			// Move to Next Page/next option on TOC
			objGLPLearnerCourseMaterialPage.clickOnElement(
					"EODetailPageNextArrow",
					"Click on next second option under TOC");

			// Verify TOC remains opened
			// Verify Module Start Button
			objGLPLearnerCourseMaterialPage.verifyElementPresent(
					"LoListActive",
					"Verify TOC remains opened on Core Instruction page");
