 ...
 * @author lekh.bahl
 * @date March 29, 2018
 * @description: Verify different question id should be rendered in Post
 *               Assessment on clicking Browser Refresh without answering the
 *               question
 */
public class GLP_330068_VerifyQuestionIdChangeOnRefresh extends BaseClass {
    public GLP_330068_VerifyQuestionIdChangeOnRefresh() {
    }

    @Test(groups = { Groups.REGRESSION, Groups.LEARNER }, enabled = true,
            description = "Verify different question id should be rendered in Post Assessment on clicking Browser Refresh without answering the question")
    public void VerifyIdChangeOnRefresh() throws Throwable {
        startReport(getTestCaseId(),
                "Verify different question id should be rendered in Post Assessment on clicking Browser Refresh without answering the question");
        ...
        try {
            ...
            // Get question ID before Exit
            String questionIDBeforeExit = objRestUtil.getQuestionIdOnUI();

            objGLPLearnerPostAssessmentPage.refreshPage();
            TimeUnit.MINUTES.sleep(1);

            // Get question ID after Exit
            String questionIDAfterRefersh = objRestUtil.getQuestionIdOnUI();

            // Verify question id change on refreshing the page
            objGLPLearnerPostAssessmentPage.verifyDifferentQuestionID(
                    questionIDBeforeExit, questionIDAfterRefersh,
                    "Verify question is getting changed if learner refersh the page");
            ...
            // Click on continue button on module test
            objGLPLearnerPostAssessmentPage.clickOnElement(
                    "ModuleTestChapter11StartButton",
                    "Click on start button of module test of first module");
            TimeUnit.MINUTES.sleep(1);
            // Get question ID after Exit
            String questionIDAfterExit = objRestUtil.getQuestionIdOnUI();

            // Verify question id change on exiting the page
            objGLPLearnerPostAssessmentPage.verifyDifferentQuestionID(
                    questionIDAfterRefersh, questionIDAfterExit,
                    "Verify question is getting changed if learner exit and again go to post assement");
