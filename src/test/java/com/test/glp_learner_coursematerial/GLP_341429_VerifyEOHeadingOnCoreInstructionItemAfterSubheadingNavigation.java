 ...
 * @date Feb 18, 2018
 * @description :To verify that EO name should be displayed on the top bar of
 *              Core Instruction Items if user navigates to EO sub headings.
 * 
 */

public class GLP_341429_VerifyEOHeadingOnCoreInstructionItemAfterSubheadingNavigation
		extends BaseClass {
	public GLP_341429_VerifyEOHeadingOnCoreInstructionItemAfterSubheadingNavigation() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that EO name should be displayed on the top bar of Core Instruction Items if user naviagtes to EO subheadings.")
	public void verifyAnswerShuffilng() {
		startReport(
				getTestCaseId(),
				"To verify that EO name should be displayed on the top bar of Core Instruction Items if user naviagtes to EO subheadings.");
		...
		try {
			...
			objGLPLearnerCourseMaterialPage.clickOnElement("SubHeadingListTwo",
					"Click on Second Subheadings");

			objGLPLearnerCourseMaterialPage.compareText(
					objGLPLearnerCourseMaterialPage.getText("EoListOne"),
					objGLPLearnerCourseMaterialPage.getText("TOCTitle"));
		}
