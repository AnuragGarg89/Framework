/*
 * PEARSON PROPRIETARY AND CONFIDENTIAL INFORMATION SUBJECT TO NDA 
 * Copyright (c) 2017 Pearson Education, Inc.
 * All Rights Reserved. 
 * 
 * NOTICE: All information contained herein is, and remains the property of 
 * Pearson Education, Inc. The intellectual and technical concepts contained 
 * herein are proprietary to Pearson Education, Inc. and may be covered by U.S. 
 * and Foreign Patents, patent applications, and are protected by trade secret 
 * or copyright law. Dissemination of this information, reproduction of this  
 * material, and copying or distribution of this software is strictly forbidden   
 * unless prior written permission is obtained from Pearson Education, Inc.
 */
package com.test.glp_instructor_performancedashboard;

import org.testng.annotations.Test;

import com.autofusion.BaseClass;
import com.autofusion.groups.Groups;
import com.autofusion.util.CommonUtil;
import com.glp.page.GLPConsole_LoginPage;
import com.glp.page.GLPInstructor_CourseViewPage;
import com.glp.page.GLPInstructor_InstructorDashboardPage;

/**
 * @author lekh.bahl
 * @date May 30, 2018
 * @description Verify the UI of the Time-on-task card on student detail view
 */
public class GLP_339194_VerifyStudentTimeOnTaskCardUI extends BaseClass {
    public GLP_339194_VerifyStudentTimeOnTaskCardUI() {
    }

    @Test(groups = { Groups.REGRESSION, Groups.INSTRUCTOR }, enabled = true,
            description = "Verify the UI of the Time-on-task card on student detail view")
    public void verifyStudentNameAndEmailIDIsDisplayed() {
        startReport(getTestCaseId(),
                "Verify the UI of the Time-on-task card on student detail view");

        try {

            GLPConsole_LoginPage objProductApplicationConsoleLoginPage = new GLPConsole_LoginPage(
                    reportTestObj, APP_LOG);
            CommonUtil objCommonUtil = new CommonUtil(reportTestObj, APP_LOG);
            objProductApplicationConsoleLoginPage.login(
                    configurationsXlsMap.get("INSTRUCTOR_USER_NAME"),
                    configurationsXlsMap.get("INSTRUCTOR_PASSWORD"));
            GLPInstructor_CourseViewPage objProductApplicationCourseViewPage = new GLPInstructor_CourseViewPage(
                    reportTestObj, APP_LOG);

            // Verify course tile on courseview page
            objProductApplicationCourseViewPage.verifyElementPresent(
                    "CourseTileInstructor",
                    "Courses associated with instruction displayed on Instructor homepage");
            GLPInstructor_InstructorDashboardPage objProductApplicationInstructorDashboardPage = new GLPInstructor_InstructorDashboardPage(
                    reportTestObj, APP_LOG);

            // Navigate to Performance dashboard
            objProductApplicationCourseViewPage
                    .navigateToPerformanceDashboard();

            // Click on Student list tab
            objProductApplicationInstructorDashboardPage.clickOnElement(
                    "InstructorDashBoardPerformanceStudentList",
                    "Click on student list tab");

            // Click on 2nd student in the student list
            objProductApplicationInstructorDashboardPage.clickOnElement(
                    "StudentListSecondStudent",
                    "Click on 2nd student in the student list");

            // Verify legend option 'student' should be displayed on time on
            // task card
            objProductApplicationInstructorDashboardPage.verifyElementPresent(
                    "TimeOnTaskStudentLegendOption",
                    "Verify legend option 'student' should be displayed on time on task card");

            // Verify legend option 'class' should be displayed on time on
            // task card
            objProductApplicationInstructorDashboardPage.verifyElementPresent(
                    "TimeOnTaskClassLegendOption",
                    "Verify legend option 'class' should be displayed on time on task card");

            // Verify time on task bar chart for student should be displayed
            objProductApplicationInstructorDashboardPage.verifyElementPresent(
                    "StudentTimeLineProgressBar",
                    "Verify time on task bar chart for student should be displayed");

            // Verify time on task bar chart for class should be displayed
            objProductApplicationInstructorDashboardPage.verifyElementPresent(
                    "ClassTimeLineProgressBar",
                    "Verify time on task bar chart for class should be displayed");

            // Click on drop downTimeOnTaskCardAllModules
            objProductApplicationInstructorDashboardPage.clickOnElement(
                    "TimeOnTaskCardAllModules",
                    "Click on All Modules time on task card drip down");

            /*
             * objProductApplicationInstructorDashboardPage.verifyTextInList(
             * "DiagnosticSubmitButton",
             * ResourceConfigurations.getProperty("submitButtonText"),
             * "Verify Submit button is displayed in " +
             * ResourceConfigurations.getProperty("language"));
             */

        } finally {
            webDriver.quit();
            webDriver = null;
        }
    }
}
