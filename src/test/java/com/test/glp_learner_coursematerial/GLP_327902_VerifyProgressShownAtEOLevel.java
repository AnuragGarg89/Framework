 ...
 * @date March 2, 2018
 * @description : To verify that student will be 'shown progress' at Enabling
 *              Objective level, as they navigate through EO Menu items in TOC.
 * 
 */
public class GLP_327902_VerifyProgressShownAtEOLevel extends BaseClass {
	public GLP_327902_VerifyProgressShownAtEOLevel() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that student will be 'shown progress' at Enabling Objective level, as they navigate through EO Menu items in TOC.")
	public void verifyProgressShownAtEOLevel() {
		startReport(
				getTestCaseId(),
				"To verify that student will be shown progress at Enabling Objective level, as they navigate through EO Menu items in TOC.");
		...
		try {
			...
			objGLPLearnerCourseMaterialPage.verifyElementPresent(
					"ProgressCircle",
					"Verify default EO progress on Core Instruction page");
