 ...
 * @date April 25, 2018
 * @description: 1. Verify that Learner should not be able to give input to any
 *               part whose all tries are exhausted. 2. Verify that Learner
 *               should not be able to view next part(s) without attempting the
 *               current part. 3. Verify that Learner should not be able to take
 *               more than the number of tries specified for a part. TC#334592 :
 *               (TC#334592 + TC#334593 + TC#334594 clubbed)
 */

public class GLP_334592_VerifyFunctionalSecurityOfMultipartInPracticeTest
        extends BaseClass {

    public GLP_334592_VerifyFunctionalSecurityOfMultipartInPracticeTest() {
    }

    @Test(groups = { Groups.REGRESSION, Groups.LEARNER, Groups.MULTIPART },
            enabled = true,
            description = "Multipart FA: Verify that Learner should not be able to give input to any part whose all tries are exhausted and not able to attempt next part untill and unless completes active part.")
    public void
           verifyStudentAbleToRetryTheCurrentQuestionAfterSubmitingResponseIncorrect() {
        startReport(getTestCaseId(),
                "Multipart FA: Verify that Learner should not be able to give input to any part whose all tries are exhausted and not able to attempt next part untill and unless completes active part.");
        ...
        try {
            ...
            int totalnumberOfParts = objGLPLearnerPracticeTestPage
            ...
            objGLPLearnerPracticeTestPage.compareText(questionOriginal,
                    ...
                    new String[] { "yes",
                            "Multipart Question before any operation",
                            "Multipart Question after second time refreshing page" });
