 ...
 * @date Feb 18, 2018
 * @description :To verify that a welcome message should not be displayed when
 *              User click on GOT IT button once for first time.
 * 
 */

public class GLP_340932_VerifyWelcomeMessageNotDisplayedAfterClikcingOnGOTIT
		extends BaseClass {
	public GLP_340932_VerifyWelcomeMessageNotDisplayedAfterClikcingOnGOTIT() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that a welcome message should not be displayed when User click on GOT IT button once for first time.")
	public void verifyAnswerShuffilng() {
		startReport(
				getTestCaseId(),
				"To verify that a welcome message should not be displayed when User click on GOT IT button once for first time.");
		...
		try {
			...
			objGLPLearnerCourseMaterialPage.verifyElementPresent(
					"TOCWelcomeGotItButton", "Verify GOT It button");

			objGLPLearnerCourseMaterialPage.clickOnElement(
					"TOCWelcomeGotItButton", "Click on GOT It button");

			objGLPLearnerCourseMaterialPage.refreshPage();

			objGLPLearnerCourseMaterialPage.verifyElementNotPresent(
					"TOCWelcomeGotItButton",
					"verify Welcome Message should not display");
