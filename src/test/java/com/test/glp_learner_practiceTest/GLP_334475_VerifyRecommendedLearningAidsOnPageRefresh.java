...
public class GLP_334475_VerifyRecommendedLearningAidsOnPageRefresh
        extends BaseClass {
    public GLP_334475_VerifyRecommendedLearningAidsOnPageRefresh() {
    }
    ...
    public void verifyLearnerCanAccessViewAnExampleLA() {
        ...
        try {
            ...
            // Attempt practice test Till HMST
            objGLPLearnerPracticeTestPage.attempPracticeTestTillHMST();

            // Navigate to question part on HMST full screen
            objGLPLearnerPracticeTestPage.navigateToQuestionOnHMST();

            // Select the answer for rendered question
            objGLPLearnerPracticeTestPage
                    .selectInteractivePartAnswerinHMSTLA(2);

            // Page Refresh
            objGLPLearnerPracticeTestPage.refreshPage();

            // Verify selection is gone after refresh
            objGLPLearnerPracticeTestPage.verifyElementPresent(
                    "PracticeTestSubmitDisableButton",
                    "Verify Disabled submit button");

            // Attempt answer in HMST
            objGLPLearnerPracticeTestPage.attemptInteractivePartinHMSTLA(2);

            // Page Refresh
            objGLPLearnerPracticeTestPage.refreshPage();

            // verify next question rendered
            objGLPLearnerPracticeTestPage.verifyElementPresent(
                    "LearningAidsContinuePartButton",
                    "Verify continue button after refresh");
