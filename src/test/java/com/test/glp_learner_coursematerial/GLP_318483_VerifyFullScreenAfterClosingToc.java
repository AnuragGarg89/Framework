 ...
 * @date Dec 18, 2017
 * @description: Verify that the Content should display on the full screen if
 *               TOC is closed by the student.
 * 
 */
public class GLP_318483_VerifyFullScreenAfterClosingToc extends BaseClass {
	public GLP_318483_VerifyFullScreenAfterClosingToc() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "Verify that the Content should display on the full screen if TOC is closed by the student.")
	public void verifyCardViewMultiSelectQuestion() {
		startReport(
				getTestCaseId(),
				"Verify that the Content should display on the full screen if TOC is closed by the student.");
		...
		try {
			...
			objGLPLearnerCourseMaterialPage.clickOnElement("DrawerIcon",
					"Click on TOC Drawer Icon");

			// Verify Video content is available and opened
			objGLPLearnerCourseViewPage.verifyElementPresent("TOCFullScreen",
					"Verify TOC in full scrren is available.");
