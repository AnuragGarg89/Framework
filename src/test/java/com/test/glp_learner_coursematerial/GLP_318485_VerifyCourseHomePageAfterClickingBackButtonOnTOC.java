 ...
 * @date Dec 18, 2017
 * @description: Verify that Student should land on course home page on
 *               selecting browser back button from TOC after landing to core
 *               instructions for the first time.
 * 
 */
public class GLP_318485_VerifyCourseHomePageAfterClickingBackButtonOnTOC extends
		BaseClass {
	public GLP_318485_VerifyCourseHomePageAfterClickingBackButtonOnTOC() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "Verify that Student should land on course home page on selecting browser back button from TOC after landing to core instructions for the first time.")
	public void verifyCourseHomePageAfterClickingBackButtonOnTOC() {
		startReport(
				getTestCaseId(),
				"Verify that Student should land on course home page on selecting browser back button from TOC after landing to core instructions for the first time.");
		...
		try {
			...
			objGLPLearnerCourseMaterialPage
					.clickBrowserBackButton("Click on Browser back button");

			// Verify Mini Module Start Button
			objGLPLearnerCourseMaterialPage
					.verifyElementPresent("MiniModuleLO",
							"Verify User navigated to course home page");
