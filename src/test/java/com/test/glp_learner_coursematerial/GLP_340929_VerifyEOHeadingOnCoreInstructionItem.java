 ...
 *              Core Instruction Items
 * 
 */

public class GLP_340929_VerifyEOHeadingOnCoreInstructionItem extends BaseClass {
	public GLP_340929_VerifyEOHeadingOnCoreInstructionItem() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that EO name should be displayed on the top bar of Core Instruction Items")
	public void verifyAnswerShuffilng() {
		startReport(
				getTestCaseId(),
				"To verify that EO name should be displayed on the top bar of Core Instruction Items");
		...
		try {
			...
			objGLPLearnerCourseMaterialPage.compareText(
					objGLPLearnerCourseMaterialPage.getText("EoListOne")
							.replace(".", ""), objGLPLearnerCourseMaterialPage
							.getText("TOCTitle"));
