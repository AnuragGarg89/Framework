 ...
 * @date March 2, 2018
 * @description :To verify that circle progress increases sequentially even on
 *              random menu item navigation under current EO
 * 
 */
public class GLP_327911_VerifyProgressIncreasesSequentiallyAfterRandomMenuNavigationAtEO
		extends BaseClass {
	public GLP_327911_VerifyProgressIncreasesSequentiallyAfterRandomMenuNavigationAtEO() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that circle progress increases sequentially even on random menu item navigation under current EO")
	public void verifyAnswerShuffilng() {
		startReport(
				getTestCaseId(),
				"To verify that circle progress increases sequentially even on random menu item navigation under current EO");
		...
		try {
			...
			String beforeNavigation = objGLPLearnerCourseMaterialPage
					.getActualEoProgressValue();
			// Verify EO Progress
			objGLPLearnerCourseMaterialPage.verifyElementPresent(
					"SubHeadingListThree",
					"Click on menu Item three of fir forst EO");

			String afterNavigation = objGLPLearnerCourseMaterialPage
					.getActualEoProgressValue();
