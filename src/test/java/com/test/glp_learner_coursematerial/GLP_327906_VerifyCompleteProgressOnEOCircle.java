 ...
 *              TOC) will be filled completely as student progress done through
 *              all core instruction
 * 
 */
public class GLP_327906_VerifyCompleteProgressOnEOCircle extends BaseClass {
	public GLP_327906_VerifyCompleteProgressOnEOCircle() {
	}

	@Test(groups = { Groups.REGRESSION, Groups.HEARTBEAT, Groups.LEARNER,
			Groups.TOC }, enabled = true, description = "To verify that EO circle (circle area on top left corner of TOC) will be filled completely as student progress done through all core instruction")
	public void verifyProgressOnEOCircle() {
		startReport(
				getTestCaseId(),
				"To verify that EO circle (circle area on top left corner of TOC) will be filled completely as student progress done through all core instruction");
		...
		try {
			...
			// Verify EO Progress
			objGLPLearnerCourseMaterialPage.completeAndVerifyTocProgress();
